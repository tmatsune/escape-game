...
#bg_img = get_image('data/assets/images/bg_0.png', [WIDTH, HEIGHT])
...
class Tile_Editor:
    ...
    def render(self):
        ...
        if self.can_place_tile:
            ...
            tile_name_text = text_surface(
                f'type: {tile_name}', 10, False, WHITE)
            self.display.blit(
                tile_name_text, [WIDTH - tile_name_text.get_width() - 10, 30])

            tile_id_text = text_surface(
                f'type: {self.tile_img_index}', 10, False, WHITE)
            self.display.blit(
                tile_id_text, [WIDTH - tile_id_text.get_width() - 10, 50])

            tile_images = self.curr_tile[2]
            self.tile_img_index %= len(
                tile_images) if len(tile_images) > 1 else 1

            img_offset = [0, 0]
            if self.tile_img_index > -1 and self.tile_img_index < len(tile_images):
                img = get_image(tile_images[self.tile_img_index], [
                                CELL_SIZE, CELL_SIZE])
                if tile_name == 'decor':
                    img = load_img(tile_images[self.tile_img_index])
        ...
        if self.can_place_tile:
            ...
            if self.mouse.left_click == Click.JUST_PRESSED or self.mouse.left_click == Click.PRESSED:
                ...
                self.level_editor.add_tile(tile_pos,
                                           [self.curr_tile[0], self.curr_tile[1], self.tile_img_index,
                                               self.curr_tile[2][self.tile_img_index]],
                                           self.layer, )
    ...
    def check_inputs(self):
        for e in pg.event.get():
            ...
            if e.type == pg.KEYDOWN:
                ...
                if e.key == pg.K_m:
                    # self.save_map_window = not self.save_map_window
