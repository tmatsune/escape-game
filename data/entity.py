...
class Entity:
    def __init__(self, data, pos, size, type, animated=False) -> None:
        self.data = data
        self.pos = pos.copy()
        self.size = size.copy()
        self.type = type
        ...
        self.tile_pos = [0, 0]
        if animated:
            self.animation_data = data.asset_manager.get_anim_data(type)
    ...
    def rect(self):
        if self.centered:
            ...
            return pg.Rect(self.pos[0] // 1, self.pos[1] // 1, self.size[0], self.size[1])
